...
/*6. List the names and ages of passengers who 
were younger than 18*/
select name,age from titanic where age<18; 
/*7. Display the number of passengers in each 
...
/*9. List the average age of passengers for each 
class*/
...
those who survived and were in 1st class*/
 select name,age from titanic where survived='servived' or pclass=1;
/*11. Find the number of passengers who paid more 
...
survive and were in 3rd class*/
 select name from titanic where survived!='survived' or pclass=3;
...
/*15. Calculate the total number of passengers on 
board*/
 select count(*) as totalpassengers from titanic;
...
/*18. Find the percentage of passengers who 
survived*/
 select (sum(case when survived='survived' then 1 else 0 end)*100.0/count(*)) as survivalpercentage from titanic;
...
those who did not survive and were in 1st class*/
 select name,age from titanic where survived='died' or pclass=1;
/*21. Find the number of passengers for each 
...
/*24. Find the average age of male and female 
passengers*/
select sex,avg(age) from titanic group by sex;
/*25. List the names of passengers who paid the 
highest fare*/
 select name,fare from titanic  order by fare  desc limit 1;
/*26. Find the number of passengers for each 
